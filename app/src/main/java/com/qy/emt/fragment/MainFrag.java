...
import com.qy.emt.R;
import com.qy.emt.adapter.wallet.MainCoinListAdapter;
import com.qy.emt.adapter.wallet.WebBannerAdapter;
import com.qy.emt.app.MyApp;
import com.qy.emt.aty.my.ExportPrikeyAty;
import com.qy.emt.aty.wallet.WalletCodeAty;
import com.qy.emt.aty.wallet.WalletMarketAty;
import com.qy.emt.aty.wallet.WalletMnemonicAty;
import com.qy.emt.aty.wallet.WalletTransactionAty;
import com.qy.emt.bean.other.database.WalletBean;
import com.qy.emt.bean.other.database.WalletCoinBean;
import com.qy.emt.bean.other.entity.WalletToken;
import com.qy.emt.bean.other.entity.WalletTokenState;
import com.qy.emt.bean.other.rxbus.ImportWalletEvent;
import com.qy.emt.bean.other.rxbus.TokenWalletEvent;
...
import com.qy.emt.fragment.main.CreateWalletInAty;
...
import com.qy.emt.utils.rxbus.ErrorBean;
import com.qy.emt.utils.rxbus.RxBusHelper;
...
//                        requestData(3);
                        ...
                        if (walletBeans.size() > 0) {
                            ...
                            Utils.Toast("The current device does not have an address");
                            return;
                        }
...
//                p.width = DisplayUtil.dip2px(this, Float.valueOf(hi)); // 宽度设置为屏幕的0.65，根据实际情况调整
                            ...
                            ppe_wallet_affirmpassword.setText("Please choose how to export your wallet");
                            ...
                            nonemic_word_id.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View view) {
                                    ...
                                    if (walletBeans == null || walletBeans.isEmpty()) {
                                        Utils.Toast("Please create an account first!");
                                        return;
                                    }
                                    if (StringUtils.isEmpty(AES256.SHA256Decrypt(walletBeans.get(0).getTheMnemonicWord(), pow.getText().toString()))) {
                                        Utils.Toast("There are no mnemonics that can be exported at the current address. Please confirm if the import method is a mnemonic or whether it is a new wallet");
                                        return;
                                    }
...
//                        getCoinPrice(SocketClient.result);
                ...
                if (!StringUtils.isEmpty(privateKey)) {
                    ...
                    Utils.Toast("wrong password! please enter again");
                }
...
//                            marketHttp();
                ...
                webBannerAdapter = new WebBannerAdapter(getActivity(), jsonArray,String.valueOf(sum));
                        ...
                        for (WalletBean walletBean : walletBeans) {
                            for (Object obj : jsonArray) {
                                ...
                                if (jsonO.getString("coinName").equals(walletBean.getCoin())) {
                                    ...
                                    if ("EMTC".equals(walletBean.getCoin())) {
